...
struct RBAABB
{
  RBVector3 min;
  RBVector3 max;
  bool operator==(const RBAABB& ab) const
  {
	  return (ab.min == min) && (ab.max == max);
  }

  RBVector3 operator[](i32 idx)
  {
	  RBVector3 d = max - min;
	  switch (idx)
	  {
	  case 0:
		  return min;
	  case 1:
		  return min + RBVector3(d.x, 0, 0);
	  case 2:
		  return min + RBVector3(0, d.y, 0);
	  case 3:
		  return min + RBVector3(0, 0, d.z);
	  case 4:
		  return min + RBVector3(d.x, d.y, 0);
	  case 5:
		  return min + RBVector3(d.x, 0, d.z);
	  case 6:
		  return min + RBVector3(0, d.y, d.z);
	  case 7:
		  return max;
	  default:
		  return min;
...
struct RBAABB2D
{
	...
	bool intersection_ray(const RBVector2& o, const RBVector2& d, float& mint, float& maxt) const
	{
		mint = 0.f;
		maxt = MAX_F32;
		for (int i = 0; i < 2; i++)
		{
			if (RBMath::abs(d[i]) < SMALLER_F)
			{
				if (o[i]<min[i] || o[i]>max[i])
				{
...
struct RBAABBI
{
  ...
  bool is_contain(const RBVector2I& v) const
  {
	  return (v.x >= min.x) && (v.x <= max.x) &&
		  (v.y >= min.y) && (v.y <= max.y);
  }
